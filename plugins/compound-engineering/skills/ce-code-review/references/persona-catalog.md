...
| `flutter` | `ce-flutter-reviewer` | `.dart` files, Flutter widgets, state-management code (Provider/Riverpod/Bloc/Cubit/GetX), `pubspec.yaml`, or the Dart side of platform channels (`MethodChannel`). Reviews the Dart/Flutter layer only — native platform-channel code (Swift/Kotlin/JS) belongs to the platform-specific personas, and `dart:ffi` native-interop safety belongs to `dart-ffi`. Do **not** spawn for generated Dart (`*.g.dart`, `*.freezed.dart`, `*.gr.dart`) or `pubspec.lock` churn alone |
| `dart-ffi` | `ce-dart-ffi-reviewer` | `dart:ffi` native interop — `Pointer`, `Struct`/`Union`, `@Native`/`Native.addressOf`, `NativeCallable`, `lookupFunction`, `DynamicLibrary`, or `package:ffi` allocators (`malloc`/`calloc`/`Arena`/`Utf8`). Reviews native memory safety, pointer lifetime, C-to-Dart type/ABI correctness, marshalling, and callback/isolate threading — orthogonal to `flutter` (widget/state) and applies to non-Flutter Dart too. Do **not** spawn for ffigen-generated bindings alone |
