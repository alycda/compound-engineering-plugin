...
| `ce-dart-ffi-reviewer` | `dart:ffi` usage — `Pointer`, `Struct`/`Union`, `@Native`, `NativeCallable`, `lookupFunction`, `DynamicLibrary`, or `package:ffi` allocators (`malloc`/`calloc`/`Arena`/`Utf8`) — **not** ffigen-generated bindings alone |
