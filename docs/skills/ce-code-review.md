...
- **Stack-specific conditional** — Julik frontend races, Swift/iOS, Flutter/Dart — only when the matching runtime domain is touched. Structural quality (complexity deletion, 1k-line regressions, spaghetti) lives in the always-on maintainability persona.
